...
POR QUE SE ENTRENA UNA VEZ POR NIVEL DE A Y NO CUATRO VECES:
  El modelo se entrena con las dos condiciones mezcladas, que es como se
  usaria una protesis real, y despues se mide por separado sobre las
  repeticiones estaticas y sobre las dinamicas. Asi el factor B es una
  particion de la evaluacion y no un cambio del entrenamiento, y las dos
  celdas de una misma fila comparten pesos: cualquier diferencia entre
  ellas viene de la condicion, no de haber entrenado con la mitad de los
  datos. Con --entrenamiento por_condicion se hace lo contrario, que es
  la pregunta de un modelo especializado por postura.
...
def main():
    ...
    p.add_argument("--entrenamiento", choices=["mixto", "por_condicion"],
                   default="mixto")
